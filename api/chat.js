...
const buildSystemPrompt = () => {
  return [
    ...
    `- If information is unavailable, say: 'I do not have that information yet. You can contact ${identity.name} via email for more details. (${identity.email})'`,
...
const buildFallbackReply = (contextText) => {
  ...
  if (lines.length === 0) {
  return `I do not have that information yet. You can contact ${identity.name} via email for more details. (${identity.email})`;
  }

  const summaryLines = lines.slice(0, 4);
 return [
  `Here is what I found from ${identity.name}'s portfolio:`,
...
const callOpenAiCompatibleModels = async ({
  ...
  return formatReplyText(reply || `I do not have that information yet. You can contact ${identity.name} via email for more details. (${identity.email})`);  
}
...
const callGemini = async ({ message, history, contextText, env }) => {
  ...
  for (const model of modelCandidates) {
    ...
    return formatReplyText(reply || `I do not have that information yet. You can contact ${identity.name}'s via email for more details. (${identity.email})`);
  }
